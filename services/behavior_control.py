"""Persistent supervisor control channel for live behavior components."""
...
from services.behavior_runtime import BehaviorModuleLoader, BehaviorRuntimeError
...
def _read_json(path: Path) -> dict[str, Any] | None:
    ...
    if path.stat().st_size > MAX_CONTROL_BYTES:
        raise BehaviorRuntimeError(f"Behavior control file is too large: {path.name}")
    value = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(value, dict):
        raise BehaviorRuntimeError("Behavior control file must contain an object")
    return value
...
class BehaviorControlWorker:
    def __init__(self, loader: BehaviorModuleLoader, control_dir: str | Path):
        self.loader = loader
        self.control_dir = Path(control_dir)
        self.request_path = self.control_dir / "behavior-request.json"
        self.result_path = self.control_dir / "behavior-result.json"
        self.active_path = self.control_dir / "active-behaviors.json"

    def active_state(self) -> dict[str, Any]:
        state = _read_json(self.active_path)
        if state is None:
            return {"version": CONTROL_VERSION, "sources": {}}
        if state.get("version") != CONTROL_VERSION or not isinstance(state.get("sources"), dict):
            raise BehaviorRuntimeError("Invalid active behavior state")
        return state

    async def restore_active(self) -> dict[str, Any]:
        restored: list[str] = []
        errors: list[str] = []
        for source_id, record in sorted(self.active_state()["sources"].items()):
            try:
    ...
    async def process_once(self) -> dict[str, Any] | None:
        ...
        if request.get("version") != CONTROL_VERSION or not isinstance(request_id, str):
            return self._failure(str(request_id or "invalid"), "Invalid behavior request")
        if not isinstance(operations, list) or not 1 <= len(operations) <= MAX_OPERATIONS:
            return self._failure(request_id, "Behavior request has invalid operations")
        ...
        try:
            for operation in operations:
                if not isinstance(operation, dict):
                    raise BehaviorRuntimeError("Behavior operation must be an object")
                ...
                if action == "activate":
                    ...
                    raise BehaviorRuntimeError(f"Unsupported behavior operation: {action!r}")
    ...
    async def _activate(self, source_id: str, record: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(relative_path, str) or not isinstance(digest, str):
            raise BehaviorRuntimeError("Activation requires relative_path and sha256")
    ...
    def _source_id(value: Any) -> str:
        if not isinstance(value, str) or not value.startswith(
            "hotbehavior:live_components/"
        ):
            raise BehaviorRuntimeError(
                "Supervisor behavior source must be beneath live_components/"
            )
        path = Path(value.removeprefix("hotbehavior:"))
        if path.suffix.lower() != ".py" or ".." in path.parts or path.name == "__init__.py":
            raise BehaviorRuntimeError("Invalid supervisor behavior source")
        return value
